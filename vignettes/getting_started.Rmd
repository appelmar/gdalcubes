...
the Moderate Resolution Imaging Spectroradiometer ([MODIS](https://lpdaac.usgs.gov/dataset_discovery/modis)) that is small enough to process locally even on older machines. These data come as a set of [HDF4](https://support.hdfgroup.org/products/hdf4/) files. Please make sure that your GDAL installation
...
The created image collection has references to original images on disk and knows about the datetime, spatial extent, and coordinate reference system of images. `data_cube()` creates a data cube from an image collection. This function takes one to three arguments:
...
```{r}
x = data_cube(image_collection("MOD11A2.db"))
x
...
v = cube_view(srs=srs, extent=extent(MOD11A2.col, srs), nx = 633, ny=413, dt="P1M")
MOD11A2.cube = data_cube(MOD11A2.col, v)
MOD11A2.cube
```

# Data cube operations

The package comes with a few operations on data cubes to (i) select bands (`select_bands`), (ii) apply pixel wise arithmetic expressions (`apply_pixel`), (iii) reduce data cubes over time (`reduce`), (iv) apply an R function over chunks of a data cube (`apply_chunk`), and (v) join bands of identically shaped data cubes (`join_bands`). 
...
MOD11A2.reduce = reduce(MOD11A2.daynight_difference, "median")
...
data_cube(image_collection("MOD11A2.db"), v) %>%
  select_bands(c("LST_DAY","LST_NIGHT")) %>%
  apply_pixel("0.02*(LST_DAY-LST_NIGHT)", names = "LST difference") %>%
  reduce("median") %>%
...
MOD11A2.cube = data_cube(image_collection("MOD11A2.db"), v)
MOD13A3.cube = data_cube(image_collection("MOD13A3.db"), v)
MOD13A3.cube
...
Data cubes are processed chunk-wise, where users can specify the size of chunks when they create a data cube from an image collection in `data_cube()`.
...
in separate R processes (data is streamed to stdin of these processes) and first must read the data as array with `read_stream_as_array()`.
This function returns a 4 dimensional array with data from one chunk (dimensions are in the order band, datetime, y, x). After doing something with the data, the result must be written to stdout using `write_stream_from_array`. In the example below, we use the joined data cube and compute
...
# Explicit evaluation
Sometimes it is be useful to store resulting data on disk. The function `write_ncdf()` can be used to force the evaluation of a data cube and write the resulting data to a NetCDF file.

```{r}
write_ncdf(reduce(joined.cube, "max"), "test.nc")
```
...
will call `write_ncdf` and load the data using the stars package.



# Multithreading
gdalcubes supports multithreaded evaluation of data cube operations. You just need to call e.g. `gdalcubes_set_threads(8)` and all following operations will use up to 8 threads. Internally, threads are assigned to chunks of result data cubes ,i.e., one chunk of the result is processed by exactly one thread. Consequently, if the result has only one chunk, multhreading will have no effect.
