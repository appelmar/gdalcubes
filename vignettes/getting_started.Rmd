...
This document presents a first brief vignette to illustrate basic ideas of the gdalcubes R package and library. 

We will process satellite imagary from the Moderate Resolution Imaging Spectroradiometer ([MODIS](https://lpdaac.usgs.gov/dataset_discovery/modis)) that is small enough even for older machines. The data come as a set of [HDF4](https://support.hdfgroup.org/products/hdf4/) files. Please make sure that your GDAL installation supports the HDF4 driver (e.g. by running `gdalinfo --format HDF4` in the command line). We assume that you have successfully installed the gdalcubes R package.

In the following, We will follow a typical workflow by

1. indexing image files in an image collection,
2. creating data cubes at various spatiotemporal resolutions and extents, 
3. apply simple operations on data cubes including band selection, application of pixel-wise functions, reduction over time, and data cube joins.


# Downloading the sample data

The vignette will use two different MODIS datasets. The first dataset contains monthly aggregated vegetation indexes over Europe from the MODIS product [MOD13A3](https://lpdaac.usgs.gov/dataset_discovery/modis/modis_products_table/mod13a3_v006), covering western Europe (tiles v=13,14, h=03,04) from January to September 2018. The zip archive has approximately 360 megabytes. The second dataset contains 8-daily land surface temperature from the MODIS product [MOD11A2](https://lpdaac.usgs.gov/dataset_discovery/modis/modis_products_table/mod11a2_v006) for the same area and time, summing to a zip archive of approximately 600 megabytes. The code below downloads and unzips the data to the current working directory. 
...
This will generate two directories MOD11A2 and MOD13A3 with HDF files from the corresponding MODIS product.


# Creating an image collection

As a first step, we need to tell gdalcubes how the products are organized in files. For MODIS data, each file is a complete image, i.e. it contains all bands. Other satellite imagery such as Landsat or Sentinel 2 can be organized quite differently with band data coming from multiple files. Furthermore gdalcubes must know how to derive the image aquisition date / time. All this needed information is specified in an _image collection format_, which is a set of regular expressions in a JSON file. gdalcubes comes with a set of predefined formats, including both MODIS products. The following command gives a list with identifiers and short descriptions of available predefined image collection formats. 
...
The created image collection has references to original images on disk and knows about the datetime, spatial extent, and coordinate reference system of images. The `raster_cube()` function creates a data cube from an image collection. This function up to three arguments:
...
code performs the same operation but might be easier to read.
...
Data cubes are processed chunk-wise, where users can specify the size of chunks as an argument to the `raster_cube()` fnuction.
...
in separate R processes and first must read the data as array with `read_chunk_as_array()`.
This function returns a 4 dimensional array with data from one chunk (dimensions are in the order band, datetime, y, x). After doing something with the data, the result must be written to stdout using `write_chunk_from_array()`. In the example below, we use the joined data cube and compute
