...
The created image collection has references to original images on disk and knows about the datetime, spatial extent, and coordinate reference system of images. `cube()` creates a data cube from an image collection. This function takes one to three arguments:
...
```{r}
x = cube(image_collection("MOD11A2.db"))
x
...
MOD11A2.cube = cube(image_collection("MOD11A2.db"), v)
MOD11A2.cube
```
...
cube(image_collection("MOD11A2.db"), v) %>%
...
MOD11A2.cube = cube(image_collection("MOD11A2.db"), v)
MOD13A3.cube = cube(image_collection("MOD13A3.db"), v)
MOD13A3.cube
...
Data cubes are processed chunk-wise, where users can specify the size of chunks when they create a data cube from an image collection in `cube()`.
...
# Explicit evaluation
Sometimes it is be useful to store resulting data on disk. The function `as_netcdf()` can be used to force the evaluation of a data cube and write the resulting data to a NetCDF file.

```{r}
as_ncdf(reduce(joined.cube, "max"), "test.nc")
```
...
will call `as_ncdf` and load the data using the stars package.
