...
## Multithreading
gdalcubes supports multithreaded evaluation of data cube operations. Calling `gdalcubes_options(threads = n)` will tell all following data cube operations to use up to `n` threads. Since worker threads are assigned to chunks of the data cube, the actual number of threads used is lower for cubes with less than `n` chunks.


# Further reading

This vignette presents a very simple example with a small dataset. Further tutorials and publications not available as a vignette inside the package but available online include:
